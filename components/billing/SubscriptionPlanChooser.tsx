    ...
    <div className="space-y-10">
      <section className="overflow-hidden rounded-lg bg-ink text-white">
        <div className="grid lg:grid-cols-[1.05fr_0.95fr]">
          <div className="p-7 sm:p-10 lg:p-12">
            <h2 className="text-2xl font-bold sm:text-3xl">Activa tu acceso profesional</h2>
            <p className="mt-3 max-w-xl leading-relaxed text-white/75">
              El contrato y la suscripción son obligatorios para aparecer activo, recibir solicitudes y responder oportunidades. Tus datos se conservan aunque canceles.
            </p>
            ...
            <Link href="/suscripcion/confirmar?plan=autonomo_nacional&interval=monthly&founder=true" className="btn btn-white mt-7 w-full sm:w-auto">
              Activar plaza fundador por 0 € <ArrowRight size={16} />
            </Link>
          </div>
          <div className="relative min-h-56 lg:min-h-full">
            <img src="/images/photos/mantenimiento.webp" alt="Técnico profesional trabajando con equipamiento industrial" className="absolute inset-0 h-full w-full object-cover" />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/70 to-transparent p-5 text-sm text-white/85 lg:hidden">
              Suscripción, contrato y actividad profesional en un único flujo.
            </div>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-ink">Elige cobertura futura</h2>
            <p className="mt-2 text-muted">Puedes revisar precio, renovación y contrato antes de ir a Stripe.</p>
          </div>
          <div className="grid grid-cols-2 rounded-lg bg-canvas p-1" aria-label="Intervalo de facturación">
            {(["monthly", "yearly"] as BillingInterval[]).map((value) => (
              <button key={value} type="button" onClick={() => setInterval(value)} className={cn("rounded-md px-4 py-2 text-sm font-semibold", interval === value ? "bg-white text-ink shadow-soft" : "text-muted")}>
                {value === "monthly" ? "Mensual" : "Anual · -10%"}
              </button>
            ))}
          </div>
        </div>
          ...
          {professionalPlans.map((plan) => {
            ...
            return (
              <article key={plan.id} className="card flex flex-col p-6 sm:p-7">
                ...
                <p className="mt-4 text-sm leading-relaxed text-ink/75">{plan.description}</p>
                ...
                <ul className="mt-5 flex-1 space-y-2">
                  {plan.features.slice(0, 4).map((feature) => <li key={feature} className="flex gap-2 text-sm text-ink/80"><Check size={15} className="mt-0.5 shrink-0 text-forest-600" />{feature}</li>)}
                </ul>
