...
/** Logotipo RegiNova: marca de obra/hoja en verde + wordmark. */
export function Logo({ className, mono = false }: { className?: string; mono?: boolean }) {
  return (
    <Link href="/" className={cn("inline-flex items-center gap-2.5 group", className)} aria-label="RegiNova — inicio">
      <span
      ...
      >
        Regi<span className={mono ? "text-mint" : "text-forest-600"}>Nova</span>
      </span>
    </Link>
  );
}
